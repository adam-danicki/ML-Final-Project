...
### Dataset loading
### -------------------------------------------
attribs, _, labels, _, _ = load_data('./Data/digits', test_size= 0.2, train_size= 0.8)
...
def strat_kfold(attribs, labels, k_val):
    ...
    for i in range(k):
        ...
        train_raw = attribs[train_index]
        test_raw = attribs[test_index]
        train_labels = labels[train_index]
        test_labels = labels[test_index]

        min_attr = train_raw.min(axis= 0)
        max_attr = train_raw.max(axis= 0)
        range_attr = np.where((max_attr - min_attr) == 0, 1, max_attr - min_attr)

        train_attribs = (train_raw - min_attr) / range_attr
        test_attribs = (test_raw - min_attr) / range_attr
